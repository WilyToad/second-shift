...
- [x] Visual check: open http://127.0.0.1:5170 (after `bun run launch -- --dev` and `bun run start`) and look at the console layout, the blueprint sketch card and the screenshot card at your monitor size (done with the player; the platform name rich text was fixed)
