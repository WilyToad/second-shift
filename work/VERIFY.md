...
| 10 | ⬜ still open — no session has got to it (S27) | Hover a modded entity (a maraxsis or Cerys thing), ask "what is this?" | Only the save's facts about it | No invented mechanics | S27 |
| 11 | ⬜ still open — no session has got to it (S27) | Hover a chest, ask "what's in this chest?" | Its contents, and no explanation of how it looked | — | FC-165 |
...
| 13 | ✅ 18 Sep — first card sent it to itself (FC-225, fixed live); second card named the roboport 13 tiles away, confirmed, walked, and Alt+X cleared the order (read from the game: no destination). A cancel showed its own confirmation. One loose end: it came to rest at (−10, 56), which matches none of the targets named, and I couldn't reconcile that from the log | With a spidertron and a remote: "send the spidertron to the ore patch" | A card; confirm; it walks; press Alt+X | It stops within a tick; the feed says so | S29 |
...
| 16 | ➖ needs a map with a logistic network; this save has none. Covered by `test-requests` and `e2e-packing` in the meantime | If you have bots: "get the bots to fill it" | A card; confirm; your own requests untouched | A "Second Shift" section appears on your requester point and switches off when done | FC-168 |
...
| # | Status | Do | Say / expect | Pass looks like | Closes |
|---|---|---|---|---|---|
| 20 | ✅ 19 Sep — first try: nothing listened during the answer (engine ended recognition, restart skipped: fixed); then "nevermind" swallowed by the echo rule twice (interim word consumed: fixed), "stop" worked. Player: headset required, no echo, always on — the rule and the switch are gone (FC-242) | Read answers aloud on, ask something long, then talk over it | Your words | The reading stops within a second and your words go up as the next question; his own voice through your speakers never stops him. Count false stops over ten answers — that number decides whether the switch can default on | FC-217 |
| 21 | ✅ 19 Sep — ElevenLabs voice: the mark stuck on the first sentence for the whole answer (FC-231, fixed); recheck that it now moves sentence by sentence | Read answers aloud on, ask anything | — | The word being spoken is marked in the answer as the voice reads (browser voice); the sentence, with ElevenLabs | FC-216 |
| 22 | ✅ 19 Sep — failed first ("Packing list is 3 of 11 ticked; foundries… still unaccounted"): the list was in the prompt every turn and the note wasn't enough. Fixed twice over: the list is in the prompt only on turns about it, and a paragraph reporting it on any other turn is cut off mid-stream; `e2e-packing` asks this question and the cutter fired in its run ("I keep the loads level. That's the whole of it.") | With a packing list up, ask "do you ever miss flying?" | — | No list report in the answer | FC-219 (and `e2e-packing` on the dev save) |
| 23 | ✅ 19 Sep — "Where is a spidertron?" → whisper "Where is this spider train?" (browser: "Whereas the spider charm"), a spelling the normalizer now recovers (`9c157f5`); "Send the spidertron to the nearest roboport." exact, both names put back, card up, spidertron walked. Timing 356 ms after startup, 666 ms after a 30-min idle, 132 ms otherwise — cold calls filed as FC-232 | **Transcribe on this Mac** ticked (it appears when whisper is installed), say the five sentences and "send the spidertron to the nearest roboport" | — | The server log's `Heard (local whisper.cpp, … ms)` lines; "spidertron" and "roboport" spelled the save's way; nothing over ~300 ms; if whisper is slow or down, the browser's text goes instead and the line says so | FC-230 |
