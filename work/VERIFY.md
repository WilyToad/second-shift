...
| 22 | ❌→ fixed 19 Sep — the answer ended "Packing list is 3 of 11 ticked; foundries, gear wheels… still unaccounted": the list was in the prompt every turn and the note wasn't enough. Now the list is in the prompt only on turns about it; `e2e-packing` asks this question. Recheck live | With a packing list up, ask "do you ever miss flying?" | — | No list report in the answer | FC-219 (and `e2e-packing` on the dev save) |
