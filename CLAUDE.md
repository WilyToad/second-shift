...
- `bun scripts/eval-barge-scorer.ts [--model test|fast|quality] [--prior] [--scoring labels]` (jevmlx in `.venv-jev/`) scores the barge-in echo rule and a Jev-style small-model scorer over the same 687 cases (FC-233). Any scorer must beat the rule here before it touches the console.
