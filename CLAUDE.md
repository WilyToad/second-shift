...
- `bun scripts/e2e-train-stops.ts` (server running, dev save hosted) finds train stops, sets their limit through a card
  and checks the game.
- `bun run sounds [-- name]` generates the console's sound effects with the ElevenLabs key into `data/sounds/`
  (skips existing ones; `--force` or a name regenerates). Uses the player's credits: only when they ask.
...
- `bun scripts/eval-barge-scorer.ts [--model test|fast|quality] [--prior] [--scoring labels]` (jevmlx in `.venv-jev/`; `--rule-only` needs no model) scores the barge-in echo rule and a Jev-style small-model scorer over the same 687 cases (FC-233, FC-234). Any scorer must beat the rule here before it touches the console.
